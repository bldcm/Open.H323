...
Q931 & Q931::operator=(const Q931 & other)
{
  ...
  facInformationElements.RemoveAll();
  ...
  for (PINDEX i = 0; i < other.facInformationElements.GetSize(); i++) {
	 facInformationElements.SetAt(other.facInformationElements.GetKeyAt(i), new PBYTEArray(other.facInformationElements.GetDataAt(i)));
  }
...
BOOL Q931::Decode(const PBYTEArray & data)
{
  ...
  while (offset < data.GetSize()) {
	...
	if(discriminator == Q931::FacilityIE) {
      facInformationElements.SetAt(facIECount, new PBYTEArray(*item));
      facIECount++;
	}
  }
...
int Q931::HasFacIE(void) const
{
  return facInformationElements.GetSize();
}
...
PBYTEArray Q931::GetFacIE(int index) const
{
  if (facInformationElements.Contains(POrdinalKey(index)))
    return facInformationElements[index];
