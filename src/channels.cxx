                                          ...
                                          unsigned & errorCode)
{
  ...
  if (!param.HasOptionalField(H245_H2250LogicalChannelParameters::e_mediaControlChannel)) {
	if (sessionID == RTP_Session::DefaultAudioSessionID) {
	    PTRACE(1, "LogChan\tNo mediaControlChannel for Audio session specified (OnReceivedPDU)");
		errorCode = H245_OpenLogicalChannelReject_cause::e_unspecified;
		return FALSE;
	}
	else {
		// Remark: Control channel is not required for Fax Sessions
	    PTRACE(2, "LogChan\tNo mediaControlChannel for none-Audio session specified (OnReceivedPDU)");
	}
  }
  else {
	remoteMediaControlAddress = param.m_mediaControlChannel;
	if (remoteMediaControlAddress.IsEmpty()) {
		PTRACE(1, "LogChan\tmediaControlChannel is empty (OnReceivedPDU)");
		return FALSE;
	}
  }

  if (!param.HasOptionalField(H245_H2250LogicalChannelParameters::e_mediaChannel)) {
    PTRACE(2, "LogChan\tNo mediaChannel specified (OnReceivedPDU)");
  }
  else {
    remoteMediaAddress = param.m_mediaChannel;
	if (remoteMediaAddress.IsEmpty()) {
		PTRACE(1, "LogChan\tmediaChannel is empty (OnReceivedPDU)");
		return FALSE;
	}
  }
...
BOOL H323_ExternalRTPChannel::OnReceivedAckPDU(const H245_H2250LogicalChannelAckParameters & param)
{
  ...
  if (!param.HasOptionalField(H245_H2250LogicalChannelAckParameters::e_mediaControlChannel)) {
	if (sessionID == RTP_Session::DefaultAudioSessionID) {
	    PTRACE(1, "LogChan\tNo mediaControlChannel for Audio session specified (OnReceivedAckPDU)");
		return FALSE;
	}
	else {
		// Remark: Control channel is not required for Fax Sessions
	    PTRACE(2, "LogChan\tNo mediaControlChannel for none-Audio session specified (OnReceivedAckPDU)");
	}
  }
  else {
	remoteMediaControlAddress = param.m_mediaControlChannel;
    PTRACE(1, "LogChan\tmediaControlChannel (OnReceivedAckPDU) = " << remoteMediaControlAddress);
	if (remoteMediaControlAddress.IsEmpty())
		return FALSE;
  }

  if (!param.HasOptionalField(H245_H2250LogicalChannelAckParameters::e_mediaChannel)) {
    PTRACE(1, "LogChan\tNo mediaChannel specified (OnReceivedAckPDU)");
    return FALSE;
  }
  else {
    remoteMediaAddress = param.m_mediaChannel;
    PTRACE(1, "LogChan\tmediaChannel (OnReceivedAckPDU) = " << remoteMediaAddress);
	if (remoteMediaAddress.IsEmpty())
		return FALSE;
  }
