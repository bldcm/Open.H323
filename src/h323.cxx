...
void H323Connection::SelectDefaultLogicalChannel(unsigned sessionID)
{
  ...
  if (IsH245Master()) {
	...
	for (PINDEX i = 0; i < remoteCapabilities.GetSize(); i++) {
		H323Capability & capability = remoteCapabilities[i];
		H323Capability * locCapability = localCapabilities.FindCapability(capability);
		if (locCapability != NULL && locCapability->GetDefaultSessionID() == sessionID) {
			PTRACE(3, "H323\tSelecting " << *locCapability);
